...
* 1.如何避免回溯？以S="abcabcd", P="abcd"为例子
  * 分情况：
    * 1.P在失配点（该点不匹配）之前**每个**字符都不相同。
      * eg:`i=3,j=3时，失配（即s到第二个a处，p到d处），之前的每个点P[0]!=P[1]!=P[2]。而失配之前的S与P相同。即P[0]=S[0],P[1]=S[1],P[2]=S[2]`此时，i的回溯不必要，不必回溯到0+1=1（下一个）进行i与j匹配，因为，`P[0]!=P[1]与P[1]=S[1]`知道`S[1]!=P[0]`.同理`P[0]!=S[2]`,所以i也不回溯到S[2],直接继续从i=3，j=0（j回溯）开始下一轮匹配。
